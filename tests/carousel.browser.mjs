...
import { configureCIPage, browserContextOptions, browserOptions, captureScreenshot, captureArtifacts } from '../scripts/browser-options.mjs';
...
try {
  ...
  const sweep = await browser.newPage(browserContextOptions); await configureCIPage(sweep); await sweep.goto('http://127.0.0.1:4196'); await sweep.waitForFunction(() => window.__littleCloud);
