...
try {
  ...
  const waitingStart = await snap();
  assert.match(waitingStart.event.run.name, /^(?:🦀|🦊|🐻|🐱|🐰|🦦|🐧|🐉) [A-Z][a-z]+$/u);
  assert.equal(waitingStart.phase, 'idle');
  assert.equal(waitingStart.event.turn, 0);
  assert.equal(waitingStart.event.remaining, 15);
  assert.equal(waitingStart.event.firstTurnPreparationElapsed, 0);
  assert.equal(waitingStart.event.firstTurnControlReady, false, 'blank synthetic video never starts the first-turn countdown');
  ...
  await page.waitForTimeout(700); assert.equal((await snap()).event.remaining, 15); assert.equal((await snap()).event.handCamera.waiting, true);
