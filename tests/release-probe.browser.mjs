...
try {
  ...
  for (const publicTry of [false, true]) {
    ...
    try {
      ...
      const options = { browser, origin, expected, cookies, hostCode, publicTry };
      ...
      await assert.rejects(verifyReleaseBrowser({ ...options, publicTry: !publicTry }), /Anonymous entry differs from EXPECTED_PUBLIC_TRY/);
      assert.deepEqual(writes, ['POST /api/login', 'POST /api/host/login'], 'Only verification authentication may reach the server');
      for (const table of ['runs', 'turns', 'public_playtest_events']) {
  ...
  console.log('Release probe: private/public Try, protected staff operator, expected-mode mismatch, zero camera/player/telemetry writes passed.');
} finally {
  await browser?.close();
