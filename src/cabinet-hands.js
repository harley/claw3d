...
export class CabinetHands {
  constructor(scene) {
    ...
    this.ready = Promise.all(['left', 'right'].map(async role => {
      const gltf = await new GLTFLoader().loadAsync(`/models/hands/${role}.glb`);
      ...
      for (const finger of fingers) for (const [i, joint] of joints.slice(1, 4).entries()) {
        ...
        bends.push({ bone, rest: bone.quaternion.clone(), axis, amount: [1.12, 1.35, .75][i] });
      }
      ...
      this.hands[role] = { pivot, bends, thumb, thumbAxis, thumbRest, curl: 0 };
  ...
  update(phase, elapsed, dt, feedback, enabled, stick, button, reduced) {
    ...
    for (const role of ['left', 'right']) {
      ...
      hand.thumb.quaternion.copy(hand.thumbRest).multiply(new T.Quaternion().setFromAxisAngle(hand.thumbAxis, (left ? .95 : -.12)*hand.curl));
      for (const bend of hand.bends) bend.bone.quaternion.copy(bend.rest).multiply(new T.Quaternion().setFromAxisAngle(bend.axis,-bend.amount*hand.curl));
      if (left) {
        hand.pivot.position.set(stick.position.x-.13,2.105,1.82);
        hand.pivot.rotation.set(stick.rotation.x*.7, -.08, stick.rotation.z*.7);
        if (!gripped) { hand.pivot.position.y += .035; hand.pivot.position.z += .12; }
      } else {
        ...
        hand.pivot.position.set(button.position.x + (strike ? .28*(1-travel) : .28), (strike ? button.position.y+.185 : 1.96)+lift, strike ? 2.0-.24*travel : 2.0);
        hand.pivot.rotation.set(reduced ? 0 : slam ? -.2*Math.sin(Math.PI*t) : -.035, .08, .04);
      }
    }
  }
}
