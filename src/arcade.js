...
function frame(time) {
  ...
  if (!frozen && !blocked && !document.hidden) {
    const effects = stepTurn(game, flow, input, dt, { preparing: Boolean(run && !recovering && !modal), reducedMotion: Boolean(scene.reducedMotion), slamSeconds: RIGHT_SLAM_MS / 1000 });
