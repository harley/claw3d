...
function createGlove(id, screen) {
  ...
  return { update(feedback, visible, dt, targets) {
  ...
  return {
    ...
    update(feedback, visible, dt) {
      ...
      if (targets) {
        ...
        const pressing = feedback.profile === 'dual' ? false : feedback.profile === 'grab-release' ? feedback.grab?.stage === 'pressing' : feedback.kind === 'clenching';
      ...
      left.update(feedback, visible && fresh && !dual && !!targets, dt, targets);
    }
  };
}
