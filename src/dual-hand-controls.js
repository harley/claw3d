...
const role = () => ({ owner: null, observed: null, candidate: null, gesture: new GrabRelease(), filter: new OneEuroPoint(), neutral: null });
const clear = state => { state.owner = state.observed = state.candidate = state.neutral = null; state.gesture.reset(); state.filter.reset(); };
...
export class DualHandControls {
  ...
  track(state, name, hands, now) {
    ...
    if (!hand.fist.open || hand.fist.closed || hand.center.x < .12 || hand.center.x > .88 || hand.center.y < .12 || hand.center.y > .86) {
    ...
    if (now - state.candidate.at >= ACQUIRE_MS) {
      state.owner = { ...hand.center }; state.candidate = null;
  ...
  update(hands, now, getTarget) {
    ...
    const leftTarget = left.ready ? getTarget(left.hand.center) : {};
    ...
    const rightTarget = right.ready ? getTarget(right.hand.center) : {};
    ...
    const view = (observation, gesture, target) => ({
    ...
    const leftView = view(left, grip, leftTarget), rightView = view(right, press, rightTarget);
    const kind = !left.ready ? left.hand ? 'calibrating' : 'lost' : !leftClear ? 'lost' :
    ...
    return { kind, input, hands: { left: leftView, right: rightView }, fired: press.fired, dropEnabled,
      ...
      message: !leftClear ? 'SHOW LEFT HAND OPEN' : !grip.steering ? 'LEFT HAND · GRAB JOYSTICK' : !right.ready || !press.armed ? 'RAISE RIGHT HAND OPEN' : 'RIGHT HAND · PRESS DROP' };
  }
}
