...
export const RULES = Object.freeze({ version: 'cloud-day-carousel-v2', turns: 3, seconds: 15, points: Object.freeze({ bonbon: 100, miso: 100, 'blue-hour': 100, peach: 100, butter: 100, sprout: 200 }) });
const supportedRules = rules => [LEGACY_RULES, RULES].some(known => JSON.stringify(known) === JSON.stringify(rules));
...
export function recordTurn(store, turn, prizeId) {
  ...
  run.turns.push({ turn, prizeId, score: prizeId ? run.rules.points[prizeId] : 0 });
...
export function loadStore(storage) {
  ...
  if (store.active) for (const [i, turn] of store.active.turns.entries()) {
    if (turn.turn !== i + 1 || (turn.prizeId !== null && !(turn.prizeId in store.active.rules.points)) || turn.score !== (turn.prizeId ? store.active.rules.points[turn.prizeId] : 0)) throw new Error('Saved turn is invalid.');
  }
  if (currentBoard(store).rules.version !== RULES.version) {
    ...
    rotateBoard(store, 'Star Carousel · Prototype');
    store.notice = 'New carousel rules: earlier scores and unfinished runs remain in the export.';
  }
  return store;
}
