...
The top-right **1 Hand | 2 Hands** toggle is available in the shared staff pilot and local preview. One hand is the default: an open hand steers and a 550 ms clench drops. Two hands: clench the left hand to grip and steer, then raise the open right hand to drop. Select before PLAY; switching reloads the selected mode without starting a run. Selection is locked during an active or interrupted run. **📷** opens camera settings; **🔊 / 🔇** controls sound, with accessible labels and hover hints.

Caught toys stay removed across all three turns in both modes. Local Continue restores caught toys; shared reload retains the existing policy of syncing completed turns and abandoning the interrupted attempt. Shared runs save their control mode and version in their immutable rules, including retries and exports. Both modes use the same scoring formula and shared fun leaderboard; local boards remain separate and historical results are preserved. Physical two-hand comfort and first-time usability remain unverified.
...
After a catch, rounds two and three show ROUND, then 3, 2, 1, START over 2.5 seconds before hand control resumes, without an extra click or using aiming time; the view returns from the shelf to close framing before START. After a miss, the empty claw lifts for 0.55 seconds and stays over its drop: the view never leaves the claw, the centre shows MISSED then START!, and the next aim begins 3.65 simulation seconds after the missed drop was accepted (1.75 seconds after MISSED appears). The next turn starts where the miss ended. Other turns start at the bed centre; after a catch the empty claw drives there from the chute during the round announcement. A catch completes its shelf delivery in 7.2 seconds and the next aim begins 9.7 seconds after acceptance. Sustained 10 FPS still advances at real time; longer frame stalls remain capped so a resumed tab cannot jump through the sequence. Camera readiness can still hold the next aiming phase.
...
The operator can pause, reset the current player, change rendering quality, start a fresh leaderboard session, and export all sessions as JSON. Quality also adapts automatically, with or without the camera: two slow five-second windows select simple rendering, which also caps scene draws at 30 per second while the camera runs and sizes any bitmap capture fallback at 320 px; six healthy windows restore full rendering at display rate. Zero-copy camera streaming stays on in both modes. The operator button pins its selected quality until the page reloads; its label shows `AUTO` only when the governor selected the mode. An explicit `?capture=N` diagnostic override remains fixed. Historical practice results remain stored but excluded from rankings. New standalone runs are labeled Local preview. Ties share rank. The leaderboard is for fun; rank does not earn an additional physical prize. Session rollover preserves old results and is blocked during an active run. Rules are saved with each run. Upgrading from earlier scoring rules starts a separate speed-score leaderboard; old results and any interrupted run remain available in the export.
