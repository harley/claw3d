...
Pushes to `main` run [Check and deploy](.github/workflows/release.yml): `npm run check:booth` (unit tests, build and eight sequential browser suites), then `npm run test:shared` and a production-container startup/backup check. Only a passing main run can deploy the checked commit to Railway. Pull requests run the same checks without production credentials or deployment. Hosted browser checks use lockfile-pinned Playwright Chromium and software WebGL for trusted test pages; they do not measure physical GPU or camera performance. Routine screenshots/video remain local review artifacts because software capture can outlast short game phases; CI preserves behavior assertions and failure diagnostics. Feature branches never deploy. A manual workflow run on main repeats the same gates.
