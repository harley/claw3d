...
Rounds two and three show ROUND, then 3, 2, 1, START before hand control resumes, without an extra click or using aiming time. The complete cue lasts 3.7 seconds, and the camera returns to the full machine while it plays. After a miss, the empty claw returns home and skips the shelf animation. The next aiming phase begins 8.5 simulation seconds after the missed drop was accepted, or 6.6 seconds after MISSED appears. Sustained 10 FPS still advances at real time; longer frame stalls remain capped so a resumed tab cannot jump through the sequence. Camera readiness can still hold the next aiming phase.
