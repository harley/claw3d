...
`?controls=dual` adds independent two-hand control in the local experiment. Show the left hand open, then clench to grip and steer. The left hand has a palm larger than the joystick and a slimmer curved forearm, with the thumb behind the fingers. Raise and hold the right hand open inside its camera window for three seconds: a filling ring and spinning rim show progress, then the virtual palm slams the domed cabinet DROP button. The right hand stays docked beside the dome instead of following tracking drift, then lifts and strikes it, following the cap as it depresses. No right fist or physical stroke is needed. Reduced motion keeps a static rim and countdown. Left/right are anatomical; model labels stay unchanged and only cursor x mirrors. Each acquisition establishes a comfortable local centre. Sticky left movement saturates steering; opening releases. Missing, stale, ambiguous or out-of-zone evidence cancels an unfinished hold. New turns require a fresh left acquisition and grip. Completed holds lock aim and score time; virtual contact accepts DROP after 360 ms, even if tracking is lost. Click DROP uses the same slam. The star cue leads acquisition, hold and contact. Exactly three turns, mode-specific browser scores and caught-toy persistence remain. The shared pilot ignores this mode. Physical-camera recognition and first-time usability remain unverified. See [product behavior](docs/PRODUCT.md#opt-in-two-hand-cabinet-controls).
