...
Pushes to `main` run [Check and deploy](.github/workflows/release.yml): a Linux production-container startup/backup check and macOS `npm run check:booth` (unit tests, build and eight sequential browser suites), followed by `npm run test:shared`. Only a passing main run can deploy the checked commit to Railway. Pull requests run the same checks without production credentials or deployment. Hosted browser checks use lockfile-pinned Playwright Chromium on standard macOS runners; Linux software rendering was too slow for phase-sensitive tests. macOS consumes more Actions minutes. CI selects the existing SIMPLE graphics setting after startup because software shadow rendering stalls phase-sensitive tests. All behavior and layout assertions remain active; full-quality visual review stays local. These checks do not establish physical GPU or camera performance. Routine screenshots/video remain local review artifacts because software capture can outlast short game phases; CI preserves behavior assertions and failure diagnostics. Feature branches never deploy. A manual workflow run on main repeats the same gates.
