...
Start the camera, enter a name, steer with one hand, then clench that hand into a fist and hold briefly to drop (open to cancel). Turns advance automatically after two seconds. The gear button opens the operator panel. There are no keyboard movement or drop controls. Sound starts off and can be enabled with the Sound button. The volume slider starts at 50%; moving it does not activate sound. Mute and zero volume immediately cancel queued notes. Short sounds follow drop, descent, grip, lift outcome and release. Text confirms an accepted drop immediately and shows catch/miss when lifting starts; points are recorded after delivery. The large countdown gains stronger contrast in the final five seconds. Reduced motion preserves a stable viewpoint and suppresses decorative celebration.
