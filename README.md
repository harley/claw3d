...
The local start screen offers **PLAY · 1 HAND** and **PLAY · 2 HANDS**. One hand is the default: either open hand steers, then a 550 ms clench drops. Two hands: the left hand must clench to grip before steering; the right opens to arm, lights DROP, then clenches over the button for 150 ms or slams. Both use the machine's wide 3D panel with a DROP progress ring and optional click. Switching modes before a run reloads into that mode and opens player selection; an active or interrupted run keeps its mode and scores. Caught toys stay removed throughout all three turns in both local modes, including after reload/Continue. Completed turns show a named trophy and points in the HUD while the 3D shelf is outside the close view. A new run restocks the machine. The two local boards remain separate. The shared pilot still uses one-hand controls until server mode metadata, scoring and physical acceptance are verified.
...
`?controls=dual` adds independent two-hand control in the local experiment. Show your left palm on your left side, hold briefly, then clench to grip and steer. Raise your right palm on your right side, hold briefly, then clench to press DROP or lift and strike downward. Each open-hand acquisition establishes a local centre at its control; you do not need to move across the screen to reach it. The machine panel is wider with controls at opposite ends. The webcam dims the area outside two L MOVE / R DROP windows. Open-palm and fist symbols guide each hand; green confirms active movement or ready DROP, while gray marks an unavailable control. Windows follow the actual hand limits and clear their active color on tracking loss. Routine two-hand instructions stay inside this view instead of covering the game. Once grabbed, the joystick stays attached through wider left-hand movement and clamps steering at full travel; opening releases it. Keep your hands on their own sides. Crossing the centre gap or leaving the camera cancels ownership; the right hand also keeps its local movement limit. Return open to reacquire. DROP tolerates a brief open-to-fist transition and small movement after a press begins on the button. On narrow screens the two-hand camera view is enlarged and centred below the machine. Right-hand loss leaves steering active; left-hand loss stops steering and cancels gesture DROP until you reopen and grab again. Closed entry, ambiguous roles and stale input cannot trigger a drop. Click DROP remains available. Both gloves are smaller; hit areas remain generous. This variant has its own browser score board. Menu gestures are unchanged. Left/right refer to your anatomical hands: model labels stay unchanged while cursor movement mirrors the camera view. Both runtimes were checked against known-hand images; physical-camera recognition, slam comfort and first-time usability still need validation.
