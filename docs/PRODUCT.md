...
Last live verification, September 12, 2026: authenticated `/build-info.json` and the rendered operator panel show clean `030a7f2` on main at [claw.coderpush.com](https://claw.coderpush.com). Railway deployment `ebfcd914-aa0e-49ad-8fd2-0a04996916fa` succeeded. Exact-commit CI passed unit/build, eight sequential booth suites, shared-session and production Linux container/backup checks. The live verifier reported no browser errors, camera access or score submission. [Release evidence and superseded status](archive/2026-09-12-five-track-release.md).

Automatic deployment still needs its GitHub production Railway token; this release used the existing authenticated CLI. Production startup verifies a database snapshot before opening the existing database. Recheck live BUILD before reporting a later release; deployment does not establish physical gesture accuracy or first-time-player acceptance.
