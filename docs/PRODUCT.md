...
## Local experiment: your hand is the claw
...
The baseline render showed competing hold indicators and a floor arc obscured by toys. This local experiment uses one dominant progress arc around the claw, stronger smooth finger tension without tremble, and up to two degrees of directional lean from actual travel. Rest, travel limits, lost control and clenching return the lean immediately to neutral; reduced motion disables lean but keeps meaningful hold progress. The small cabinet glove is secondary and disappears during a hold. The remote progressbar remains available to assistive technology without a visible duplicate meter.

The claw pivot and floor target remain exact; contact uses the original mechanical pose. Recognition, 550 ms hold, physics, timing, scoring, audio and three-turn rules are unchanged. Acceptance for the local comparison: deliberate steering stays calm at rest, and a held fist clearly closes the claw beside one progress cue at ordinary viewing distance. Synthetic rendering and regressions cannot establish fun or physical gesture accuracy. Compare against the baseline on the intended camera/display before accepting this experiment; it is not deployed.
