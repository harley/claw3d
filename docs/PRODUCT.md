...
- **Local audition — movement music:** Min requested upbeat music during hand control. The development-only `?music=1` audition plays quiet original synthesis during active camera aiming; no notes are queued ahead. Camera waiting, drops, round announcements, dialogs, host pause, hidden pages, mute and zero volume stop it. It is excluded from the production build. Acceptance before deployment: Harley listens while playing, confirms that drop cues remain distinct, and checks that staff can converse comfortably.
