...
Camera recognition and the controlling-hand highlight remain visible during setup and delivery, but only aiming accepts game actions. Calibration uses a fixed anchor; the highlighted hand is labeled YOU and its neutral point is shown. Captures older than 300 ms, out of order, or from a stopped/restarted/hidden camera generation cannot control the game. Owner loss beyond 650 ms requires stable single-hand acquisition again. Frames are not recorded.
