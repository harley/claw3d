...
Sound starts off and requires an explicit Sound-button click. The visible slider cannot activate sound; its default is 50% of the previous level and its maximum is the previous maximum. Mute and zero volume cancel queued notes. Brief acceptance, descending tones, grip clicks, rising catch or falling miss tones at lift, release chimes and the final catch melody follow the actual game phases. Drop acceptance text updates immediately; catch/miss text appears when lifting starts, about 2.1 seconds after acceptance, instead of waiting for delivery to finish. Misses keep truthful return text through reveal. Points are still recorded only at turn completion. Star beeps require the visible, active near-ring cue; they stay quiet during pause, dialogs, hidden pages and camera waiting. No continuous music or new sound assets are used. Physical speaker audibility and comfortable staff conversation still need testing; organizer permission for an external speaker is unconfirmed.
