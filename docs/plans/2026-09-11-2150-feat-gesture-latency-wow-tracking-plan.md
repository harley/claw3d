...
Track 3 implemented 2026-09-11 (branch `feat/track3-wow-pass`), all five items: hold-progress arc in the target ring (amber→green, progress-driven so it survives reduced motion) with claw pre-tension against a copied pose (contacts keep the untensioned pose); one-draw-call instanced confetti burst at lift (140 particles + gold for the star, 80 otherwise); an 11-bulb instanced marquee row (delivery chase, jackpot strobe, idle attract chase — repainted only when the lit pattern changes); idle-only attract mode (toys take turns waving, star jelly ripples, pulsing invitation copy gated by `prefers-reduced-motion`); and an 8.5 % camera lean-in during `descend`/`grip` only — aim/idle still snap to home, preserving no-drift-while-aiming. Adversarial review confirmed and fixed 3 defects pre-merge (undefined-feedback crash in the carousel clearance sweep, burst surviving a mid-flight reduced-motion toggle, marquee re-uploading instance colors every frame).

Track 4 implemented 2026-09-11 (branch `feat/track4-dead-code`): `src/clasp.js` deleted; `handle()` in `src/vision.js` reduced to the single live gesture system (open hand steers, held fist drops) — pinch-steering, open-palm drop, palm/clasp branches, the `getProfile` plumbing, and the `gripping`/`dropArmed`/`openSince` state all removed; legacy `moveClaw`/`findCatch`/`FIELD` deleted from `src/mechanics.js` (which keeps `pinchRatio`/`joystickAxis`/`matchHand` and now takes `clamp` from `arcade-mechanics.js` — the duplicated `clamp`/`ease` are gone). Tests exercising only deleted paths were removed; the two generic invariants they carried (two-hand acquisition block, single-frame handedness-flip continuity) were converted into `tests/fist.test.mjs`. The client telemetry enum no longer lists `clasping`/`dropping`; the server enum keeps them for retained old-client rows.

Track 5 implemented 2026-09-11 (branch `feat/track5-perf-structure`). Hot-path cleanup: one `dialog[open]` query per frame shared by the frame loop, `updateUI` and `phaseSound` (event-driven callers keep a live default); the UI change signature joins plain fields instead of `JSON.stringify`; the camera overlay canvas resizes only on aspect change instead of clearing via width reassignment every draw; `ToyContacts.rock` reuses each obstacle's Box3 while its world matrix is unchanged (~10 full scene-graph traversals per rocked frame become 16-float comparisons). Structure: `arcade.js` 583 → 430 lines with three focused modules — `arcade-audio.js` (WebAudio lifecycle + synthesis), `arcade-hud.js` (announcement surface, cue, meters, chips behind `update(view, feedback, modal)`), `shared-board.js` (session API lifecycle, board cache, role, status, refresh/rotation versioning) — behaviour unchanged. Transmission measurement (synthetic, headless SwiftShader — relative cost only, NOT booth evidence): the star jelly's transmission pass costs **+115 draw calls per frame** (402 vs 287) and ~3× scene-update CPU; the draw-call delta is renderer-independent. Product decision (Harley, 2026-09-12): the jelly is removed entirely — it read as out of place beside the plush toys, and the pass cost was structural. The stars now wear a glossy candy finish (clearcoat + sheen); the squish wave stays as jackpot feedback, the interior bubbles and the star-only transparent-face compositing hack went with the refraction. Default draw calls: 402 → 285 per frame.

All five tracks are implemented. Outstanding physical items: booth-hardware latency/feel playtest (Track 1), 320-vs-640 accuracy A/B (Track 1), HandLandmarker miss-rate comparison (Track 1 step 6), wow-pass delight check (Track 3), transmission decision is CLOSED (jelly removed product-wide).
