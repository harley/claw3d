...
- Merge when the fast lane and the browser checks are green and the PR body is complete; routine integration is authorized and does not wait for human review. Use focused review (or `/code-review`) for consequential changes and resolve material findings first. A PR that touches camera, phase, scoring, collision or server code must have run `npm run check:booth` and `npm run test:shared` locally, sequentially. Keep PRs small enough to review in one sitting; split otherwise.
