...
- Arm `gh pr merge --squash --auto` as soon as the PR is complete; the merge queue runs the full checks on the merged group and lands it without anyone watching. Do not run the full `check:booth` locally before every PR: run the unit tests and only the browser suites you touched, and let CI run the rest. Use focused review (or `/code-review`) for consequential changes and resolve material CodeRabbit findings; the rest can follow up. Keep PRs small enough to review in one sitting, but bundle changes that touch the same files into one PR rather than serial PRs that rebase on each other.
