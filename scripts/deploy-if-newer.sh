...
set -euo pipefail
sha="$1"
# Prefer what is actually serving: the authenticated live build. The BUILD_COMMIT
# variable is written before the upload, so after a failed upload it can name a
# commit that never went live; use it only when the site cannot be read.
live=$(node scripts/live-build.mjs 2>/dev/null || true)
source="live build"
if [ -z "$live" ]; then
  ...
  source="BUILD_COMMIT variable (live build unreadable)"
fi
if [ -z "$live" ]; then echo "Live build unknown; deploying $sha." >&2; echo current=true; exit 0; fi
echo "Comparing against $source $live." >&2
